...
app.post('/api/resume', upload.single('resume'), async (req, res) => {
  try {
    ...
    const resumePath = path.join('/tmp/uploads', req.file.filename);
    
    console.log('Resume uploaded to:', resumePath); // Add logging for file path
    ...
    console.log('ATS Score calculated:', atsScore); // Log ATS score

    res.json({ score: atsScore, role });
  } catch (error) {
    console.error('Error processing the resume:', error);  // Log the full error
    res.status(500).json({ message: 'Error processing the resume. Try again later.' });
  }
});
